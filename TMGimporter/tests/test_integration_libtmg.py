"""Integration tests for libtmg.py — require a Gramps SQLite database.

Split from test_libtmg.py because in-memory Gramps DB init hangs on Windows
under the pip-installed Gramps + conda-forge GTK stack. Linux CI picks these
up via the test_integration*.py glob.

Run with:
    python -m pytest tests/test_integration_libtmg.py
