"""Integration tests for the TMG importer.

Two kinds of tests live here:
  * TestFullPipelineRelph — full end-to-end pipeline against a real SQZ backup
    (skipped automatically when TMG_TEST_SQZ / integration_test.sqz is absent).
  * Everything else — DB-backed unit-style tests for individual libtmg
    functions using mocked DBF tables.

All tests need a real Gramps SQLite database; CI picks them up via the
test_integration*.py glob on Linux only (the Windows unit job skips them
because in-memory Gramps DB init hangs under the pip-installed Gramps +
conda-forge GTK stack).

Run with:
    python3 -m pytest TMGimporter/tests/test_integration.py -v

The SQZ path is read from the environment variable TMG_TEST_SQZ. If not set
it defaults to the backup used during development.
"""
...
from gramps.gen.lib import Event, NoteType, Person, Place, Source
...
# ---------------------------------------------------------------------------
# Helpers shared across the function-level DB tests
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Full end-to-end pipeline against the real SQZ backup
# ---------------------------------------------------------------------------
