"""Unit tests for libtmg.py — pure-logic only.

Tests that require a Gramps database live in test_integration_libtmg.py.
These tests run everywhere (Linux, Windows) without touching the DB stack.

Run with:
    python -m pytest tests/test_libtmg.py
"""
